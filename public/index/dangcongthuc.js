...
function initializeAddRecipe() {
    ...
    document.getElementById('video_file')?.addEventListener('change', (event) => {
        ...
        if (file?.type === 'video/mp4') {
            ...
            document.getElementById('remove_video_btn').classList.remove('hidden');
        }
    });
    ...
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                showAdminNotification(recipeId ? "Cập nhật công thức thành công!" : "Đăng công thức thành công! Vui lòng chờ duyệt.", "success");
                setTimeout(() => {
                    window.location.href = '/cong-thuc-cua-toi';
                }, 3500);
            } catch (err) {
