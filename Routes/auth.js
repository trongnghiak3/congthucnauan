...
router.post("/dang-nhap", async (req, res) => {
    ...
    try {
        ...
        if (!results.length) {
            return res.render("index/dang-nhap", {
                title: "Đăng Nhập",
        ...
        // Đúng mật khẩu -> tạo session
        req.session.user = {
            ID_CHINH_ND: nguoiDung.ID_CHINH_ND,
        ...
        return res.redirect(nguoiDung.VAI_TRO === "admin" ? "/admin" : "/");
    } catch (err) {
        console.error("Lỗi truy vấn:", err);
        return res.render("index/dang-nhap", {
            title: "Đăng Nhập",
            notify: { type: "error", message: "Lỗi server. Vui lòng thử lại!" }
        });
    }
});
...
router.post("/dang-ky", async (req, res) => {
  ...
  if (!name || !email || !password || !confirmPassword) {
    return res.render("index/dang-ky", {
      notify: { type: "error", message: "Vui lòng nhập đầy đủ thông tin." },
    });
  }
  ...
  if (password !== confirmPassword) {
    return res.render("index/dang-ky", {
      notify: { type: "warning", message: "Mật khẩu xác nhận không khớp." },
    });
  }
  ...
  if (exists) {
    return res.render("index/dang-ky", {
      notify: { type: "error", message: "Email đã được sử dụng." },
    });
  }
  ...
  try {
    ...
    await query(
      "INSERT INTO nguoi_dung (TEN_NGUOI_DUNG, EMAIL_, MAT_KHAU, VAI_TRO, TRANG_THAI) VALUES (?, ?, ?, ?, ?)",
      [name, email, hash, "nguoidung", "hoatdong"]
    );

    // Đăng ký thành công, chuyển sang đăng nhập
    res.redirect("/dang-nhap?notify=registered");
  } catch (err) {
    console.error("Lỗi lưu người dùng:", err);
    res.render("index/dang-ky", {
      notify: { type: "error", message: "Đã xảy ra lỗi phía server. Vui lòng thử lại." },
    });
  }
});
